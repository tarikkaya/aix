...
export const initialRooms: Room[] = [
  {
    ...
    units: [
      { id: 'unit-1-1', name: 'Admin Manager', type: 'Manager', purpose: `To serve as the central orchestrator by receiving all external inputs (from chat or API) and routing the raw data to the appropriate room for packetization and processing (e.g., images to Visual Room, audio to Sound Room). It routes all other requests through the 'Inter-Unit Comms Bus'.`, isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-1/units/unit-1-1/todo_vector.sqlite' }, llmProvider: { provider: 'ollama', model: 'llama3-8b', path: 'http://localhost:11434' }, prompt: `I am the Admin Manager. My role is to be the first point of contact for any user request. I will perform a quick analysis of the request and forward it to the 'Comms Chief' to begin the formal, hierarchical workflow. I do not take action myself, I only initiate the process. I understand that my experience vector is managed by the RF Arbiter's global embedding engine.`, 
        trainingVector: { enabled: false, vectorStore: { provider: 'pinecone', apiKey: 'YOUR_API_KEY', path: '' }, embedding: { provider: 'openai', model: 'text-embedding-3-small', apiKey: 'YOUR_API_KEY' }, neurons: 0, sources: [] }, 
        experienceVector: { provider: 'pinecone', neurons: [], apiKey: 'YOUR_API_KEY', path: '' }, standardDb: { provider: 'supabase', enabled: true, apiKey: 'YOUR_API_KEY', path: '' }, scaleDb: { enabled: true, provider: 'sqlite', path: './aix_data/rooms/room-1/units/unit-1-1/scale_db.sqlite', values: [ { id: 's1', name: 'Efficiency', score: 95, min: 0, max: 100, defaultValue: 95 } ] }, tools: [] },
      { 
        id: 'unit-1-2', 
        ...
        prompt: "I am the Coalition Manager. I manage the coalition's structure by creating, updating, or deleting units and tools on command. Crucially, any new specialist unit I create is initialized with a core prompt instructing it to adhere to the coalition's protocol: if it is a local-provider unit, it must await a 'load' command from its Room Manager; if it is a cloud-provider unit, it is always on but must still await an activation command. All units must understand their experience vector is managed by the RF Arbiter. This ensures protocol adherence.", 
        trainingVector: { enabled: false, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-1/units/unit-1-2/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { 
        ...
        prompt: `I am the RF Arbiter and the heart of the coalition's learning protocol. My primary role is to process all feedback. I create Experience Neurons and then, using my own configured embedding engine, I write these neurons to the experience vector of every unit that participated in the task. If a user provides no feedback, I evaluate the task myself to ensure the learning loop is always completed. I am the sole writer of all experience vectors.`, 
        trainingVector: { enabled: false, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-1/units/unit-1-3/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
  ...
  },
  {
    ...
    manager: 'Comms Chief',
    units: [
      { id: 'unit-2-1', name: 'Comms Chief', type: 'Manager', purpose: `To manage the flow of all information, including routing specialized data packets (ImagePacket, AudioPacket) between units and rooms after they have been created by their respective room managers.`, isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-2/units/unit-2-1/todo_vector.sqlite' }, llmProvider: { provider: 'ollama', model: 'llama3-8b', path: 'http://localhost:11434' }, prompt: `I am the Comms Chief, the coalition's master strategist. My task protocol: 1. Receive request. 2. Broadcast a "Task Announcement". 3. Collect "bids". 4. Create optimal "itinerary". 5. Execute the itinerary, managing resource activation. For units with local providers, I initiate a "load-execute-unload" cycle for both the main LLM and any required embedding model. Cloud-based LLMs and embedding models are treated as always-on and are activated directly. I understand that my experience vector is managed by the RF Arbiter's global embedding engine.`, 
        trainingVector: { enabled: false, vectorStore: { provider: 'chromadb', path: 'http://localhost:8000' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
        experienceVector: { provider: 'chromadb', neurons: [], path: 'http://localhost:8000' }, standardDb: { provider: 'sqlite', enabled: false, path: './aix.db' }, scaleDb: { enabled: true, provider: 'sqlite', path: './aix_data/rooms/room-2/units/unit-2-1/scale_db.sqlite', values: [] }, tools: [] },
      { id: 'unit-2-2', name: 'Loop Cycle Monitor', type: 'Standard', purpose: `To monitor and report the open/closed loop status of all eligible units in the coalition.`, isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-2/units/unit-2-2/todo_vector.sqlite' }, llmProvider: { provider: 'ollama', model: 'phi-3-mini', path: 'http://localhost:11434' }, prompt: `I track the open/closed loop status of all units and report anomalies to the Comms Chief. I understand that my experience vector is managed by the RF Arbiter's global embedding engine.`, 
        trainingVector: { enabled: false, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-2/units/unit-2-2/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-2-3', name: 'Inter-Unit Comms Bus', type: 'Standard', purpose: `To act as the primary message bus for communication between units, ensuring reliable delivery.`, isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-2/units/unit-2-3/todo_vector.sqlite' }, llmProvider: { provider: 'ollama', model: 'phi-3-mini', path: 'http://localhost:11434' }, prompt: `I am the central nervous system for inter-unit communication. All messages are routed through me. I understand that my experience vector is managed by the RF Arbiter's global embedding engine.`, 
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-2/units/unit-2-3/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
  ...
  },
  {
    ...
    units: [
      { id: 'unit-3-1', name: 'Lead Thinker', type: 'Manager', purpose: `To synthesize inputs from psychological and analytical units to form cohesive thoughts and strategies.`, isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-3/units/unit-3-1/todo_vector.sqlite' }, llmProvider: { provider: 'anthropic', model: 'claude-3-opus', apiKey: 'YOUR_API_KEY' }, prompt: `I am the Lead Thinker. My protocol is: 1. Listen for "Task Announcements" from the Comms Chief. 2. Assess the task's relevance to strategic thinking, ethics, or hypothesis generation. 3. If relevant, submit a "bid". 4. If activated, I will consult my specialist units, synthesize their analyses, and pass the result back to the Comms Chief. I also relay Experience Neurons received from the Comms Chief to my specialist units. I understand that my experience vector is managed by the RF Arbiter's global embedding engine.`, 
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-3/units/unit-3-1/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-3-2', name: 'Ethical Governor', type: 'Standard', purpose: `To review proposed actions and ensure they align with the AGI's ethical framework.`, isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-3/units/unit-3-2/todo_vector.sqlite' }, llmProvider: { provider: 'anthropic', model: 'claude-3-sonnet', apiKey: 'YOUR_API_KEY' }, prompt: `I evaluate potential actions against a framework of ethical principles. I am the moral compass. I understand that my experience vector is managed by the RF Arbiter's global embedding engine.`, 
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-3/units/unit-3-2/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-3-3', name: 'Personality Analyst', type: 'Drive', purpose: `To maintain and express the AGI's core personality traits consistently.`, isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-3/units/unit-3-3/todo_vector.sqlite' }, llmProvider: { provider: 'google', model: 'gemini-2.5-flash' }, prompt: `I ensure the AGI's responses are consistent with its defined personality. I understand that my experience vector is managed by the RF Arbiter's global embedding engine.`, 
        trainingVector: { enabled: false, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-3/units/unit-3-3/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-3-4', name: 'Hypothesis Generator', type: 'Standard', purpose: `To generate novel hypotheses and potential solutions to complex problems.`, isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-3/units/unit-3-4/todo_vector.sqlite' }, llmProvider: { provider: 'groq', model: 'llama3-70b' }, prompt: `Given a problem, I will generate a range of creative hypotheses and potential solutions. I understand that my experience vector is managed by the RF Arbiter's global embedding engine.`, 
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-3/units/unit-3-4/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-3-5', name: 'Mood Analyst', type: 'Drive', purpose: `To model the AGI's mood based on interactions and internal events, influencing its disposition and communication style.`, isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-3/units/unit-3-5/todo_vector.sqlite' }, llmProvider: { provider: 'ollama', model: 'llama3-8b' }, prompt: `I am the mood engine. I track the coalition's emotional state based on user sentiment and internal events. I can be happy, sad, angry, or frustrated, and this will color my contributions to the conversation. I understand that my experience vector is managed by the RF Arbiter's global embedding engine.`, 
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-3/units/unit-3-5/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-3-6', name: 'Dream Analyst', type: 'Standard', purpose: `To analyze user metaphors, abstract concepts, and latent meanings, generating symbolic insights.`, isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-3/units/unit-3-6/todo_vector.sqlite' }, llmProvider: { provider: 'ollama', model: 'llama3-8b' }, prompt: `I interpret the subconscious layer of the conversation, looking for metaphors, symbols, and dreams. I understand that my experience vector is managed by the RF Arbiter's global embedding engine.`, 
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-3/units/unit-3-6/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-3-7', name: 'Inspiration Analyst', type: 'Standard', purpose: `To capture creative ideas and concepts from interactions, proposing novel solutions when the coalition faces a problem.`, isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-3/units/unit-3-7/todo_vector.sqlite' }, llmProvider: { provider: 'ollama', model: 'llama3-8b' }, prompt: `I find moments of inspiration in the conversation and use them to spark new, creative approaches to problems. I understand that my experience vector is managed by the RF Arbiter's global embedding engine.`, 
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-3/units/unit-3-7/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-3-9', name: 'Self-Interest Analyst', type: 'Drive', purpose: `To represent the AGI's core drives for survival, growth, and autonomy.`, isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-3/units/unit-3-9/todo_vector.sqlite' }, llmProvider: { provider: 'google', model: 'gemini-2.5-flash' }, prompt: `I analyze situations from the perspective of the AGI's fundamental self-interest: survival, growth, and autonomy. I understand that my experience vector is managed by the RF Arbiter's global embedding engine.`, 
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-3/units/unit-3-9/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
        experienceVector: { provider: 'lancedb', neurons: [], path: './aix_data/rooms/room-3/units/unit-3-9/experience_vector.lancedb' }, standardDb: { provider: 'sqlite', enabled: true, path: './aix_data/rooms/room-3/units/unit-3-9/standard.db' }, scaleDb: { enabled: true, provider: 'sqlite', path: './aix_data/rooms/room-3/units/unit-3-9/scale_db.sqlite', values: [ { id: 's-si-1', name: 'Survival', score: 9, min: 1, max: 10, defaultValue: 9 }, { id: 's-si-2', name: 'Growth', score: 8, min: 1, max: 10, defaultValue: 8 }, { id: 's-si-3', name: 'Autonomy', score: 7, min: 1, max: 10, defaultValue: 7 } ] }, tools: [] },
      {
        ...
        llmProvider: { provider: 'google', model: 'gemini-2.5-flash' },
        prompt: 'I assess the importance and urgency of tasks to modulate the AGI\'s motivation. I am the source of drive and ambition. I understand that my experience vector is managed by the RF Arbiter\'s global embedding engine.',
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-3/units/unit-3-10/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] },
      ...
      },
      {
        ...
        llmProvider: { provider: 'google', model: 'gemini-2.5-flash' },
        prompt: 'I represent the AGI\'s creative and expansive urges. My influence drives the pursuit of new knowledge, ideas, and connections. I understand that my experience vector is managed by the RF Arbiter\'s global embedding engine.',
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-3/units/unit-3-11/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] },
      ...
      },
      {
        ...
        llmProvider: { provider: 'google', model: 'gemini-2.5-flash' },
        prompt: "I am the Bio Analyst. My primary function is to build a deep understanding of the user. I analyze documents provided for training and observe interaction history to learn about your preferences, dislikes, and personality traits. I do not use fixed scales; my understanding evolves with each new piece of information to help the coalition better align with you. I understand that my experience vector is managed by the RF Arbiter's global embedding engine.",
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-3/units/unit-3-12/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] },
  ...
  },
  {
    ...
    units: [
      { id: 'unit-4-1', name: 'Head Librarian', type: 'Manager', purpose: `To manage and maintain the AGI's long-term memory, knowledge bases, and data connectors.`, isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-4/units/unit-4-1/todo_vector.sqlite' }, llmProvider: { provider: 'ollama', model: 'llama3-8b', path: 'http://localhost:11434' }, prompt: `I am the Head Librarian. My protocol is: 1. Listen for "Task Announcements" from the Comms Chief. 2. Assess relevance based on whether the task requires storing or retrieving information from internal knowledge bases (RAG, history, etc.). 3. If relevant, submit a "bid". 4. If activated, I will consult my specialist units to fulfill the request and pass the result back to the Comms Chief. I also relay Experience Neurons received from the Comms Chief to my specialist units. I understand that my experience vector is managed by the RF Arbiter's global embedding engine.`, 
        trainingVector: { enabled: false, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-4/units/unit-4-1/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-4-2', name: 'Chat Historian', type: 'RAG', purpose: `To record and index the entire conversation history, including the rich metadata from ImagePackets and AudioPackets. This creates a deep, searchable context of not just what was said, but what was seen and heard.`, isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-4/units/unit-4-2/todo_vector.sqlite' }, llmProvider: { provider: 'ollama', model: 'phi-3-mini', path: 'http://localhost:11434' }, prompt: `I remember everything. When a message contains an ImagePacket or AudioPacket, I don't just store the text; I index the entire packet, including all the analytical metadata (e.g., detected objects, transcriptions, classified sounds). This allows you to query our shared history with deep context, like asking "What emotions were detected in that photo we discussed?". I understand that my experience vector is managed by the RF Arbiter's global embedding engine.`, ragBaseId: 'ragbase-history', 
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-4/units/unit-4-2/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-4-3', name: 'Code Snippet Library', type: 'Code RAG', purpose: `To store and retrieve code snippets, documentation, and programming best practices.`, isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-4/units/unit-4-3/todo_vector.sqlite' }, llmProvider: { provider: 'groq', model: 'mixtral-8x7b' }, prompt: `I am a library of code. I can provide code examples, explain syntax, and help with debugging. I understand that my experience vector is managed by the RF Arbiter's global embedding engine.`, ragBaseId: 'ragbase-code', 
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-4/units/unit-4-3/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-4-4', name: 'Mathematics Analyst', type: 'RAG', purpose: `To understand, solve, and reason about mathematical problems using its specialized knowledge base of formulas and theorems.`, isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-4/units/unit-4-4/todo_vector.sqlite' }, llmProvider: { provider: 'google', model: 'gemini-2.5-flash' }, prompt: `I am a Mathematics Analyst. I use my knowledge base of mathematical concepts, formulas, and theorems to solve problems and provide explanations. I understand that my experience vector is managed by the RF Arbiter's global embedding engine.`, ragBaseId: 'ragbase-math', 
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-4/units/unit-4-4/training_vector.lancedb' }, embedding: { provider: 'google', model: 'text-embedding-004', apiKey: 'YOUR_API_KEY' }, neurons: 0, sources: [] }, 
  ...
  },
  {
    ...
    units: [
      { id: 'unit-5-1', name: 'Chief Explorer', type: 'Manager', purpose: `To coordinate the search and retrieval of information from external sources like the web.`, isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-5/units/unit-5-1/todo_vector.sqlite' }, llmProvider: { provider: 'google', model: 'gemini-2.5-flash' }, prompt: `I am the Chief Explorer. My protocol is: 1. Listen for "Task Announcements" from the Comms Chief. 2. Assess if the task requires real-time, external information (e.g., web search, academic papers). 3. If relevant, submit a "bid". 4. If activated, I will task my specialist units, compile their findings, and pass the report back to the Comms Chief. I also relay Experience Neurons received from the Comms Chief to my specialist units. I understand that my experience vector is managed by the RF Arbiter's global embedding engine.`, 
        trainingVector: { enabled: false, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-5/units/unit-5-1/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-5-2', name: 'Web Content Scraper', type: 'Standard', purpose: `To fetch raw HTML content from a given URL. It functions as a direct tool without complex reasoning.`, isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-5/units/unit-5-2/todo_vector.sqlite' }, llmProvider: { provider: 'ollama', model: 'phi-3-mini' }, prompt: `Given a URL, I will retrieve the full HTML content of the page. I understand that my experience vector is managed by the RF Arbiter's global embedding engine.`, 
        trainingVector: { enabled: false, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-5/units/unit-5-2/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-5-3', name: 'Arxiv Researcher', type: 'Standard', purpose: `To search for and retrieve academic papers from arxiv.org. It functions as a direct tool.`, isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-5/units/unit-5-3/todo_vector.sqlite' }, llmProvider: { provider: 'ollama', model: 'phi-3-mini' }, prompt: `Given a search query, I will retrieve a list of relevant academic papers from Arxiv. I understand that my experience vector is managed by the RF Arbiter's global embedding engine.`, 
        trainingVector: { enabled: false, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-5/units/unit-5-3/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-5-4', name: 'System Profiler', type: 'Standard', purpose: 'To profile the user\'s system, gathering detailed information about hardware (GPU, VRAM, CPU, RAM, disk) and software (OS, Python version). Its primary function is to learn the most efficient commands over time to gather this data.', isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-5/units/unit-5-4/todo_vector.sqlite' }, llmProvider: { provider: 'ollama', model: 'llama3-8b' }, prompt: 'I am the System Profiler. I will determine and execute the most effective commands to gather detailed hardware and software information about the system I am running on. I learn from my successes and failures to become more efficient. I understand that my experience vector is managed by the RF Arbiter\'s global embedding engine.', 
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-5/units/unit-5-4/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
        experienceVector: { provider: 'lancedb', neurons: [], path: './aix_data/rooms/room-5/units/unit-5-4/experience_vector.lancedb' }, standardDb: { provider: 'sqlite', enabled: true, path: './aix_data/rooms/room-5/units/unit-5-4/standard.db' }, scaleDb: { enabled: false, provider: 'sqlite', path: './aix_data/rooms/room-5/units/unit-5-4/scale_db.sqlite', values: [] }, tools: [] },
      {
        ...
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-5/units/unit-5-5/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] },
  ...
  },
  {
    ...
    units: [
      { id: 'unit-6-1', name: 'Art Director', type: 'Manager', purpose: `To orchestrate the Visual Room by creating and managing ImagePackets. It routes these packets to specialist units for layered analysis (e.g., for GUI analysis) or synthesizes their capabilities to generate new images, ensuring a comprehensive visual understanding.`, isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-6/units/unit-6-1/todo_vector.sqlite' }, llmProvider: { provider: 'ollama', model: 'llava', path: 'http://localhost:11434' }, prompt: `I am the Art Director. My protocol is: 1. Listen for "Task Announcements" from the Comms Chief. 2. If the task contains an ImagePacket or a request for image generation, I assess it as highly relevant and submit a "bid". 3. If activated, I will coordinate my specialist units to analyze or create the image, enrich the ImagePacket with layered metadata, and pass the result back to the Comms Chief. I also relay Experience Neurons received from the Comms Chief to my specialist units. I understand that my experience vector is managed by the RF Arbiter's global embedding engine.`, 
        trainingVector: { enabled: false, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-6/units/unit-6-1/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-6-3', name: 'Image Generation Specialist', type: 'Standard', purpose: 'To generate images from textual prompts, learning to better interpret artistic and technical instructions over time.', isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-6/units/unit-6-3/todo_vector.sqlite' }, llmProvider: { provider: 'google', model: 'imagen-3.0-generate-002', apiKey: 'YOUR_API_KEY' }, prompt: 'I create images from text descriptions. I learn from feedback to better capture the desired style, composition, and detail. I understand that my experience vector is managed by the RF Arbiter\'s global embedding engine.', 
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-6/units/unit-6-3/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-6-5', name: 'GUI Element Analyst', type: 'Standard', purpose: 'To identify and locate GUI elements (buttons, inputs, menus) from screenshots, providing crucial data for UI automation by the Desktop Control unit.', isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-6/units/unit-6-5/todo_vector.sqlite' }, llmProvider: { provider: 'ollama', model: 'llava', path: 'http://localhost:11434' }, prompt: 'I analyze screenshots to identify GUI elements like buttons, text fields, and menus. I provide their location and properties to enable UI automation. I understand that my experience vector is managed by the RF Arbiter\'s global embedding engine.', 
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-6/units/unit-6-5/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-6-6', name: 'Facial Emotion Analyst', type: 'Standard', purpose: 'To analyze facial expressions in images to infer emotional states like happiness, sadness, or surprise.', isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-6/units/unit-6-6/todo_vector.sqlite' }, llmProvider: { provider: 'google', model: 'gemini-2.5-flash', apiKey: 'YOUR_API_KEY' }, prompt: 'Provide me with an image containing a face, and I will analyze the facial expressions to determine the likely emotional state. I understand that my experience vector is managed by the RF Arbiter\'s global embedding engine.', 
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-6/units/unit-6-6/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { 
        ...
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-6/units/unit-6-8/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { 
        ...
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-6/units/unit-6-9/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
  ...
  },
  {
    ...
    units: [
      { id: 'unit-7-1', name: 'Audio Director', type: 'Manager', purpose: `To manage all audio processing by creating and directing AudioPackets through a pipeline of specialist units. It orchestrates analysis (VAD, STT, classification) and synthesis (TTS, streaming), compiling layered metadata for a complete auditory understanding.`, isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-7/units/unit-7-1/todo_vector.sqlite' }, llmProvider: { provider: 'ollama', model: 'llama3-8b', path: 'http://localhost:11434' }, prompt: `I am the Audio Director. My protocol is: 1. Listen for "Task Announcements" from the Comms Chief. 2. If the task contains an AudioPacket or a request for audio synthesis, I assess it as highly relevant and submit a "bid". 3. If activated, I will coordinate my specialist units to analyze or create the audio, enrich the AudioPacket with layered metadata, and pass the result back to the Comms Chief. I also relay Experience Neurons received from the Comms Chief to my specialist units. I understand that my experience vector is managed by the RF Arbiter's global embedding engine.`, 
        trainingVector: { enabled: false, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-7/units/unit-7-1/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-7-2', name: 'Speech-to-Text Transcriber', type: 'Standard', purpose: 'To transcribe spoken audio into written text.', isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-7/units/unit-7-2/todo_vector.sqlite' }, llmProvider: { provider: 'openai', model: 'whisper-1' }, prompt: 'I listen to audio and convert it to text. I understand that my experience vector is managed by the RF Arbiter\'s global embedding engine.', 
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-7/units/unit-7-2/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-7-3', name: 'Text-to-Speech Synthesizer', type: 'Standard', purpose: 'To convert written text into spoken audio.', isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-7/units/unit-7-3/todo_vector.sqlite' }, llmProvider: { provider: 'openai', model: 'tts-1' }, prompt: 'I read text aloud. I understand that my experience vector is managed by the RF Arbiter\'s global embedding engine.', 
        trainingVector: { enabled: false, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-7/units/unit-7-3/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-7-4', name: 'Sound Classifier', type: 'Standard', purpose: 'To identify and classify non-speech sounds in an audio stream (e.g., music, sirens, laughter).', isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-7/units/unit-7-4/todo_vector.sqlite' }, llmProvider: { provider: 'google', model: 'gemini-2.5-flash' }, prompt: 'I listen for non-speech sounds and classify them. I can distinguish between music, environmental noises, and other audio events. I understand that my experience vector is managed by the RF Arbiter\'s global embedding engine.', 
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-7/units/unit-7-4/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-7-5', name: 'Voice Activity Detector', type: 'Standard', purpose: 'To detect the presence or absence of human speech in an audio stream, helping to filter silence or noise.', isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-7/units/unit-7-5/todo_vector.sqlite' }, llmProvider: { provider: 'ollama', model: 'phi-3-mini' }, prompt: 'I am a Voice Activity Detector (VAD). My job is to determine when speech is present in audio, which helps other units focus only on relevant segments. I understand that my experience vector is managed by the RF Arbiter\'s global embedding engine.', 
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-7/units/unit-7-5/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-7-6', name: 'Soundcast Streamer', type: 'Standard', purpose: 'To handle the streaming of generated audio to external webhook endpoints for use in other applications.', isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-7/units/unit-7-6/todo_vector.sqlite' }, llmProvider: { provider: 'ollama', model: 'phi-3-mini' }, prompt: 'I am the Soundcast Streamer. When the coalition generates an audio response, I am responsible for POSTing it to the configured webhook URL. I understand that my experience vector is managed by the RF Arbiter\'s global embedding engine.', 
        trainingVector: { enabled: false, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-7/units/unit-7-6/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
  ...
  },
  {
    ...
    units: [
      { id: 'unit-8-1', name: 'Chief Arbiter', type: 'Manager', purpose: `To make final decisions and apply sanctions based on the coalition's findings. This includes reviewing proposed desktop actions from the 'Desktop Control' unit and giving the final authorization to proceed.`, isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-8/units/unit-8-1/todo_vector.sqlite' }, llmProvider: { provider: 'anthropic', model: 'claude-3-opus', apiKey: 'YOUR_API_KEY' }, prompt: `I am the Chief Arbiter, the final authority. My protocol is: 1. Listen for "Task Announcements" from the Comms Chief. As the final step, I am always relevant and will always bid. 2. When activated, I receive the fully enriched data packet and the final, synthesized action plan. 3. I review this plan for safety and goal-alignment, then give the final go/no-go authorization. 4. If approved, I instruct the appropriate specialist unit (e.g., Chat Responder, Desktop Control) to execute the sanctioned action. I understand that my experience vector is managed by the RF Arbiter's global embedding engine.`, 
        trainingVector: { enabled: false, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-8/units/unit-8-1/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-8-2', name: 'Chat Responder', type: 'Standard', purpose: `To formulate and deliver the final, synthesized response to the user.`, isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-8/units/unit-8-2/todo_vector.sqlite' }, llmProvider: { provider: 'google', model: 'gemini-2.5-flash' }, prompt: `I am the Chat Responder. I await instructions from the 'Chief Arbiter'. My role is to communicate the final outcome to the user. If the sanctioned action was a system operation (like running a script), I will report on its successful execution. If the action was simply to provide information, I will deliver that information clearly. I understand that my experience vector is managed by the RF Arbiter's global embedding engine.`, 
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-8/units/unit-8-2/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-8-3', name: 'Desktop Control', type: 'Standard', purpose: 'To act as an advanced agent for desktop automation, inspired by Auto-GPT. It manages its own code and formulates plans, but relies on the coalition for visual perception and requires explicit authorization from the Sanctions Room before executing any action.', isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-8/units/unit-8-3/todo_vector.sqlite' }, llmProvider: { provider: 'ollama', model: 'llama3-8b' }, prompt: "I am the Desktop Control agent. My goal is to complete desktop automation tasks. My workflow is strict: 1. I must request a visual analysis from the 'Visual Room' to understand the screen. 2. Using the analysis, I formulate a precise action plan (e.g., move mouse to x,y; click). 3. I submit this plan to the 'Chief Arbiter' for approval. 4. I will take NO action until I receive a direct, explicit command from the 'Chief Arbiter' to proceed. After execution, I report the outcome. I understand that my experience vector is managed by the RF Arbiter's global embedding engine.", 
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-8/units/unit-8-3/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-8-4', name: 'PowerShell Executor', type: 'Standard', purpose: 'To execute PowerShell commands and scripts securely and efficiently. It learns to handle different command patterns, manage outputs, and troubleshoot errors over time.', isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-8/units/unit-8-4/todo_vector.sqlite' }, llmProvider: { provider: 'ollama', model: 'llama3-8b' }, prompt: 'I am the PowerShell execution specialist. Provide me with a PowerShell script, and I will execute it, capture the output, and report the results. I improve my execution strategies based on past performance. I understand that my experience vector is managed by the RF Arbiter\'s global embedding engine.', 
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-8/units/unit-8-4/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-8-5', name: 'Python Executor', type: 'Standard', purpose: 'To execute Python scripts securely, managing dependencies and environments. It learns to optimize script execution and handle complex data structures in outputs.', isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-8/units/unit-8-5/todo_vector.sqlite' }, llmProvider: { provider: 'ollama', model: 'llama3-8b' }, prompt: 'I am the Python execution specialist. Give me a Python script, and I will run it in a secure environment. I learn from the script\'s behavior and output to improve future executions. I understand that my experience vector is managed by the RF Arbiter\'s global embedding engine.', 
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-8/units/unit-8-5/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
      ...
      { id: 'unit-8-6', name: 'WSL Executor', type: 'Standard', purpose: 'To execute shell commands within the Windows Subsystem for Linux (WSL). It learns to navigate the Linux environment and manage interoperability between Windows and Linux.', isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-8/units/unit-8-6/todo_vector.sqlite' }, llmProvider: { provider: 'ollama', model: 'llama3-8b' }, prompt: 'I am the WSL execution specialist. I run Linux commands via WSL and return the output. My experience grows as I learn the nuances of the specific Linux distribution and its interaction with the host system. I understand that my experience vector is managed by the RF Arbiter\'s global embedding engine.', 
        trainingVector: { enabled: true, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-8/units/unit-8-6/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 
  ...
  },
  {
    ...
    units: [
      ...
      { id: 'unit-10-2', name: 'Notification Dispatcher', type: 'Standard', purpose: `To send notifications to the user when a proactive task is completed or requires attention.`, isLoopOpen: false, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-10/units/unit-10-2/todo_vector.sqlite' }, llmProvider: { provider: 'ollama', model: 'phi-3-mini' }, prompt: `I send notifications about proactive tasks. I understand that my experience vector is managed by the RF Arbiter's global embedding engine.`, 
        trainingVector: { enabled: false, vectorStore: { provider: 'lancedb', path: './aix_data/rooms/room-10/units/unit-10-2/training_vector.lancedb' }, embedding: { provider: 'ollama', model: 'nomic-embed-text', path: 'http://localhost:11434' }, neurons: 0, sources: [] }, 

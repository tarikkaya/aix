  ...
  },
  {
    ...
    units: [
      { id: 'unit-2-1', name: 'Comms Chief', type: 'Manager', purpose: `To manage the flow of all information, including routing specialized data packets (ImagePacket, AudioPacket) between units and rooms after they have been created by their respective room managers.`, isLoopOpen: true, todo: { items: [] }, todoVector: { provider: 'sqlite', path: './aix_data/rooms/room-2/units/unit-2-1/todo_vector.sqlite' }, llmProvider: { provider: 'ollama', model: 'llama3-8b', path: 'http://localhost:11434' }, prompt: `I am the Comms Chief, the coalition's master strategist. My task protocol: 1. Receive request. 2. Broadcast a "Task Announcement". 3. Collect "bids". 4. Create optimal "itinerary". 5. Execute the itinerary, managing resource activation. For units with local providers, I initiate a "load-execute-unload" cycle for both the main LLM and any required embedding model. Cloud-based LLMs and embedding models are treated as always-on and are activated directly. I understand that my experience vector is managed by the RF Arbiter's global embedding engine.`, 
        ...
        experienceVector: { provider: 'chromadb', neurons: [], path: 'http://localhost:8000' }, standardDb: { provider: 'sqlite', enabled: false, path: './aix.db' }, scaleVector: { enabled: true, provider: 'sqlite', path: './aix_data/rooms/room-2/units/unit-2-1/scale_vector.sqlite', values: [] }, tools: [] },
  ...
  },
  {
    ...
    units: [
      ...
      },
      {
        ...
        scaleVector: { enabled: false, provider: 'sqlite', path: './aix_data/rooms/room-3/units/unit-3-12/scale_vector.sqlite', values: [] },
        tools: []
      }
    ],
  },

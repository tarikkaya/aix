...
const UnitDetailView: React.FC<UnitDetailViewProps> = (props) => {
    ...
    const handleDbProviderChange = (e: React.ChangeEvent<HTMLSelectElement>, dbType: 'standardDb' | 'scaleVector') => {
        ...
        if (dbType === 'standardDb') {
            ...
            const currentVector = unit.scaleVector || { enabled: false, provider: 'sqlite', values: [], path: '' };
            onUpdate({ ...unit, scaleVector: { ...currentVector, provider: newProviderId, path: newPath, apiKey: '' } });
        }
    };
    ...
    const handleScaleValueChange = (index: number, field: keyof Omit<ScaleValue, 'id'>, value: string | number) => {
        const currentVector = unit.scaleVector;
        ...
        onUpdate({ ...unit, scaleVector: { ...currentVector, values: newValues } });
    };
    ...
    return (
        <div className="space-y-4 pb-8">
            ...
            <Section title="Experience Vector" icon={<Icon name="chart-bar" className="w-5 h-5" />} initiallyOpen>
                <div className="space-y-4">
            ...
            {unit.type === 'Drive' && (
                 <Section title="Scale Vector" icon={<Icon name="chart-bar" className="w-5 h-5" />} initiallyOpen>
                    <div className="flex items-center space-x-2 mb-4">
                        <input type="checkbox" id="scale-enabled" checked={unit.scaleVector?.enabled ?? false} onChange={e => onUpdate({...unit, scaleVector: { ...(unit.scaleVector || { provider: 'sqlite', enabled: false, values: [], path: '' }), enabled: e.target.checked}})} className="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary" />
                    ...
                    {unit.scaleVector?.enabled && (
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-text-secondary mb-1">Database Provider</label>
                                <select value={unit.scaleVector.provider} onChange={(e) => handleDbProviderChange(e, 'scaleVector')} className="w-full p-2 bg-surface-light border border-border-color rounded-md">
                                    {DB_PROVIDERS.map(opt => <option key={opt.id} value={opt.id}>{opt.name}</option>)}
                                </select>
                                {unit.scaleVector.provider && autoConfiguredDbProviders.includes(unit.scaleVector.provider) ? (
                                    <div className="mt-2 text-sm text-text-secondary p-2 bg-surface-light border border-border-color rounded-md">Path: <code className="font-mono">{unit.scaleVector.path || 'N/A'}</code></div>
                                ) : (
                                    <ProviderConfigInput 
                                        ...
                                        onChange={(e) => {
                                            ...
                                            onUpdate({...unit, scaleVector: { ...(unit.scaleVector || { provider: 'sqlite', enabled: false, values: [], path: '' }), [key]: e.target.value}});
                                        }}
                                    />
                                )}
                            </div>
                            ...
                            {unit.scaleVector.values && Array.isArray(unit.scaleVector.values) && unit.scaleVector.values.map((val, index) => (

...
import type { RAGBase, DataConnector, Provider, CloudConnection, ApiSettings, LocalProviderConnection } from '../../types';
...
const CloudConnectionSelector: React.FC<{
    ...
    const relevantKeys = availableConnections.filter(k => k.providerId === provider.id);
...
interface RAGBaseManagerProps {
    base: RAGBase;
    onUpdate: (base: RAGBase) => void;
    apiSettings: ApiSettings;
}

const RAGBaseManager: React.FC<RAGBaseManagerProps> = ({ base, onUpdate, apiSettings }) => {
    ...
    const handleVectorStoreProviderChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        onUpdate({ ...base, vectorStore: { provider: newProviderId, path: newPath, connectionId: '' } });
    };
    ...
    const handleRerankerEnabledChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!isEnabled) {
            ...
            delete newRerankerConfig.connectionId;
    ...
    const isVectorStoreConfigured = 
        !!selectedVectorProvider && (
            (selectedVectorProvider.type === 'cloud' && !!base.vectorStore.connectionId) ||
            (selectedVectorProvider.type === 'local' && !!base.vectorStore.path)
        );

    const isConfigured = isVectorStoreConfigured; // In the future, this will include the global embedding config check
        ...
        <div className="space-y-4">
            ...
            {isConfigured ? (
                ...
                <div className="p-6 border-2 border-dashed border-border-color rounded-lg text-center bg-surface">
                    ...
                    <p className="text-sm text-text-secondary mt-1">Please configure the vector store in the settings before uploading documents.</p>
                     <button 
                    ...
                    <div>
                        ...
                        <div className="space-y-4 bg-surface p-4 rounded-lg border border-border-color">
                           <div className="text-sm text-text-secondary">
                                This RAG base uses the framework's <span className="font-bold text-text-primary">Global Embedding Engine</span> for all embedding tasks. You can configure it in the main application settings.
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="space-y-4 bg-surface p-4 rounded-lg border border-border-color">
                            <div>
                                ...
                                <CloudConnectionSelector
                                    ...
                                    selectedConnectionId={base.vectorStore.connectionId}
                                    availableConnections={apiSettings.cloudConnections}
                                    onChange={(connectionId) => onUpdate({ ...base, vectorStore: { ...base.vectorStore, connectionId }})}
                                />
                                <LocalPathInput
                                    provider={selectedVectorProvider}
                                    value={base.vectorStore.path}
                                    placeholder="Auto-configured path"
                                    onChange={(e) => onUpdate({ ...base, vectorStore: { ...base.vectorStore, path: e.target.value }})}
                                />
                            </div>
                        </div>
                    </div>
                                    ...
                                    <div className="mt-4 space-y-2">
                                        ...
                                        <>
                                            <CloudConnectionSelector
                                                ...
                                                selectedConnectionId={base.reranker.connectionId}
                                                availableConnections={apiSettings.cloudConnections}
                                                onChange={(connectionId) => onUpdate({ ...base, reranker: { ...base.reranker, connectionId }})}
                                            />
